...
//# Revision     : $Rev:: 65                                                      $ #
//# Author       : $Author::                                                      $ #
//# File-ID      : $Id:: Watchdog.cs 65 2023-12-21 22:58:05Z                      $ #
//#                                                                                 #
//###################################################################################
using PGA.wpEventLog;
using PGA.WebAutomation;
using PGA.Helferlein;
using System;
...
namespace PGA.PlugIns {
	public class Watchdog {
		private PLog eventLog;
		private System.Timers.Timer watchdogTimer;
		private int watchdogByte;
		private int maxWatchdogByte;
		public Watchdog() {
			eventLog = new PLog(EventLogSrc.PlugInWatchdog);
			eventLog.Write("PlugIn Watchdog gestartet");
			int watchdogVerz;
			if (Int32.TryParse(WebAutomationServer.getINI("Watchdog", "Verz"), out watchdogVerz)) {
				...
				if (!Int32.TryParse(WebAutomationServer.getINI("Watchdog", "MaxInt"), out maxWatchdogByte)) {
					maxWatchdogByte = 255;
				}
		...
		private void setWDB() {
			...
			int watchdogId;
			if (Int32.TryParse(WebAutomationServer.getINI("Watchdog", "DpId"), out watchdogId)) {
				Program.MainProg.POPCClient.setValue(watchdogId, watchdogByte.ToString(),
					WebAutomationServer.TransferWatchdog);
				PDebug.Write("write WatchdogByte: {0}", watchdogByte);
			} else {
				string[] ids = WebAutomationServer.getINI("Watchdog", "DpId").Split(',');
	...
	public class PServiceStatus {
		...
		public PServiceStatus(string servicename) {
			eventLog = new PLog(EventLogSrc.PlugInServiceStatus);
			this._serviceName = servicename;
			if (_serviceName != "" && checkServiceInstalled()) {
				serviceTimer = new Timer(Timer_Tick, _serviceName, 0, 500);
				if (PHelp.isAdmin()) {
					PDebug.Write("Autostart Services activated.");
