...
//# Revision     : $Rev:: 213                                                     $ #
//# Author       : $Author::                                                      $ #
//# File-ID      : $Id:: Shopping.cs 213 2025-05-15 14:50:57Z                     $ #
//#                                                                                 #
//###################################################################################
	...
	public class Shopping {
		...
		private static void setGroupProductChecked(bool isChecked, int idGroup, int idProduct) {
			using(Database Sql = new Database("Set Shopping Product checked")) {
				Sql.NonResponse($"UPDATE [shoppinggroupproduct] SET [ok] = {(isChecked ? 1 : 0)} WHERE [id_group] = {idGroup} AND [id_product] = {idProduct}");
			}
		...
		private static void setProductChecked(bool isChecked, int idProduct) {
			using(Database Sql = new Database("Set Shopping Product checked")) {
				Sql.NonResponse($"UPDATE [shoppinglistproduct] SET [ok] = {(isChecked ? 1 : 0)} WHERE [id_list] = 1 AND [id_product] = {idProduct}");
			}
